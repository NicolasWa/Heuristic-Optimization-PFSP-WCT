...
long int PfspInstance::computeWCT(vector< int > & sol)
{
	...
	int jobNumber;
	long int wct;
	int nbJobsSequence = sol.size()-1;
	/* We need end times on previous machine : */
	// vector< long int > previousMachineEndTime ( nbJob + 1 );

	vector< long int > previousMachineEndTime ( nbJobsSequence + 1 );
...
vector<int> PfspInstance::computeInitVectorHeuristics(){
	/* Computes the initial solution of the instance with the Simplified RZ heuristic */
	int i, j, m;
	float sumProcessTime;
  	std::vector <int> start(nbJob + 1);
	std::vector <float> temp(nbJob + 1);
	std::vector<pair<float, int> > vectorIndexBasedSorting;

	for (j = 1; j <= nbJob; ++j){
		sumProcessTime = 0;
		for (m = 1; m <= nbMac; ++m) {
			sumProcessTime+= processingTimesMatrix[j][m];
		}
		temp[j]= sumProcessTime/priority[j];
		vectorIndexBasedSorting.push_back(make_pair(temp[j], j));
	}
	sort(vectorIndexBasedSorting.begin(), vectorIndexBasedSorting.end());
	for (i=1; i<=nbJob; i++){
		start[i] = vectorIndexBasedSorting[i-1].second;
  	}
	return start;
 }

 float PfspInstance::getSumProcessingTime(){
	 int j, i;
	 float sumProcessingTime =0;
	 for (j = 1; j <= nbJob; ++j){
		for (i = 1; i <= nbMac; ++i) {
			sumProcessingTime+= processingTimesMatrix[j][i];
		}
	 }
